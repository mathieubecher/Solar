...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] PlatformNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is PlatformNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (PlatformNetworkObject.Length > 0 && PlatformNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(PlatformNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<PlatformBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiatePlatform instead, its shorter and easier to type out ;)")]
		public PlatformBehavior InstantiatePlatformNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlatformNetworkObject[index]);
			var netBehavior = go.GetComponent<PlatformBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<PlatformBehavior>().networkObject = (PlatformNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of Platform
		/// </summary>
		/// <returns>
		/// A local instance of PlatformBehavior
		/// </returns>
		/// <param name="index">The index of the Platform prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public PlatformBehavior InstantiatePlatform(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(PlatformNetworkObject[index]);
			var netBehavior = go.GetComponent<PlatformBehavior>();
