...
public class CameraController : MonoBehaviour
{
    ...
    {
        float angle = (transform.eulerAngles.x + 180) % 360 - 180;
        if (angle > upMaxAngle) transform.eulerAngles = new Vector3(upMaxAngle,transform.eulerAngles.y,transform.eulerAngles.z);
        if (angle < bottomMaxAngle) transform.eulerAngles = new Vector3(bottomMaxAngle,transform.eulerAngles.y,transform.eulerAngles.z);
    }
}
